...
namespace POSMainForm
{
    public partial class frmListStaff : Form
    {
        ...
        public void LoadStaffs(string search)
        {
            try
            {
                SQLConn.sqL = "SELECT StaffID, CONCAT(Lastname, ', ', Firstname) as ClientName, CONCAT(Street, ', ', City , ', ', Province) as Address, ContactNo, username, role FROM Staff WHERE LASTNAME LIKE '" + search.Trim() + "%' ORDER By Lastname";
                SQLConn.ConnDB();
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            if (Interaction.MsgBox("Are you sure you want to Delete?", MsgBoxStyle.YesNo, "Delete Product") == MsgBoxResult.Yes)
            {
                ...
                try
                {
                    SQLConn.sqL = "DELETE FROM staff WHERE StaffID = '" + staffID + "'";
                    SQLConn.ConnDB();
                    SQLConn.cmd = new SqlCommand(SQLConn.sqL, SQLConn.conn);
